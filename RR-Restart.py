...
def restart_system():
    ...
    try:
        if os_name == "Windows":
            ...
            print(Fore.RED + "Unsupported OS for restart command." + Style.RESET_ALL)
    except Exception as e:
        print(Fore.RED + f"Error restarting system: {e}" + Style.RESET_ALL)
...
print(Fore.RED + "Pick the correct number, and your PC will restart immediately! 😈" + Style.RESET_ALL)
print(Fore.MAGENTA + "*" * 104 + Style.RESET_ALL)
time.sleep(0.5)

# Randomly choose the "restart number"
restart_number = random.randint(1, 6)
...
for attempt in range(1, 4):
    ...
    # If the guess matches the restart number, initiate restart
    if guess == restart_number:
        print(Fore.RED + "💀 You chose the correct number! Your PC will now restart..." + Style.RESET_ALL)
        # Optionally, play the alarm sound before restarting
        play_alarm()
        restart_system()
        break
    else:
...
# If the player did not trigger a restart during their attempts
if guess != restart_number:
